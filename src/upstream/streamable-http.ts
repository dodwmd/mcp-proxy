...
/**
 * StreamableHTTP-based upstream connection.
 * Uses HTTP POST for sending messages and HTTP GET with Server-Sent Events for receiving messages.
 */
export class StreamableHTTPUpstream implements IUpstreamHandle {
  ...
  constructor(
    ...
    if (!config.url) {
      throw new Error(`StreamableHTTP server ${config.alias} missing url`);
    }

    // Validate keepalive configuration
    if (keepaliveMs < 0) {
      throw new Error(`StreamableHTTP server ${config.alias} has invalid keepaliveMs: ${keepaliveMs} (must be >= 0)`);
    }
  ...
  async init(clientInfo: { name: string; version: string }): Promise<void> {
    ...
    // Create StreamableHTTP transport with disabled auto-reconnect
    this.transport = new StreamableHTTPClientTransport(url, {
      requestInit: this.config.headers ? { headers: this.config.headers } : undefined,
      reconnectionOptions: {
        maxRetries: 0,  // Disable auto-reconnect as per requirements
