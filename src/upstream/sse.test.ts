...
describe('SSEUpstream', () => {
  let upstream: SSEUpstream | null = null;
  ...
  describe('Constructor Validation', () => {
    it('should throw when URL is missing', () => {
      ...
      expect(() => {
        new SSEUpstream(config, 'session-123', false, 30000);
    ...
    it('should accept valid public URL', () => {
      ...
      expect(() => {
        upstream = new SSEUpstream(config, 'session-123', false, 30000);
      }).not.toThrow();
    ...
    it('should accept private URL when allowPrivateUrls is true', () => {
      ...
      expect(() => {
        upstream = new SSEUpstream(config, 'session-123', true, 30000);
      }).not.toThrow();
    });
    ...
    it('should set serverId, alias, and sessionId from config', () => {
      ...
      upstream = new SSEUpstream(config, 'session-456', false, 30000);
  ...
  describe('Keepalive Configuration', () => {
    it('should accept keepalive interval of 0 to disable keepalive', () => {
      ...
      expect(() => {
        upstream = new SSEUpstream(config, 'session-123', false, 0);
      }).not.toThrow();
    });
    ...
    it('should accept positive keepalive interval', () => {
      ...
      expect(() => {
        upstream = new SSEUpstream(config, 'session-123', false, 60000);
      }).not.toThrow();
    });
    ...
    it('should reject negative keepalive interval', () => {
      ...
      expect(() => {
        new SSEUpstream(config, 'session-123', false, -1);
    ...
    it('should use fake timers to verify keepalive timer execution', async () => {
      ...
      upstream = new SSEUpstream(config, 'session-123', false, 1000);
  ...
  describe('Legacy Transport Notice', () => {
    it('should be marked as deprecated in favor of StreamableHTTP', () => {
      // This test serves as documentation that SSE is legacy
      // The actual deprecation is in JSDoc comments
      const config: ResolvedServerConfig = {
        id: 'test-1',
        alias: 'test',
        name: 'Test Server',
      ...
      // SSE should still work, but is deprecated
      expect(() => {
        upstream = new SSEUpstream(config, 'session-123', false, 30000);
      }).not.toThrow();
    });
